...
describe('interception is disabled', async () => {
  ...
  it('requests are not intercepted', async () => {
    ...
    expect(content).toMatch(/page_request_time\{path=\"\/\"}\ 0/gm)
    expect(content).toMatch(/page_request_time\{path=\"\/a\"}\ 0/gm)
    expect(content).toMatch(/page_request_time\{path=\"\/b\"}\ 0/gm)
